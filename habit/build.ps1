...
示例:
  .\build-docker.ps1                           # 自动检测代理
  .\build-docker.ps1 -Proxy force              # 强制使用代理
  .\build-docker.ps1 -Proxy none               # 不使用代理
  .\build-docker.ps1 -Registry myregistry -ImageName myapp
  .\build-docker.ps1 -NoPush -Proxy force
"@
    exit 0
}
...
if (-not (Test-Path ".\package.json")) {
    Write-Host "错误: 当前目录下未找到 package.json 文件" -ForegroundColor Red
    exit 1
}
